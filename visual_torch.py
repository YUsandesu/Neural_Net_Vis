...
network_flow = {
    ...
    "Transformer Example": [
        {
            "left_dis": 100,
            "up_dis": 100,      # Transformer 模块较大，增加垂直间距
            "shape": "rect",
            "color": (255, 180, 100),
            "count": 9,  
            "draw_size": [40, 90], 
...
def settings():
    ...
    for name, groups in network_flow.items():
        ...
        u_dis = groups[0].get("up_dis", 40) # 取该层第一个元素的 up_dis，默认40
        
        # 计算该层理论上的总高度： (节点数 - 1) * 垂直间距
    ...
    calc_height = int(max(500, max_h_needed + 150)) # 上下留出缓冲边距
...
def calculate_coordinates():
    ...
    for i, (name, groups) in enumerate(network_flow.items()):
        l_dis = groups[0].get("left_dis", default_x_spacing)
        u_dis = groups[0].get("up_dis", 40) # 提取垂直间距
    ...
    for i in range(len(layers_data) - 1):
        ...
        for n_node in next_l:
            ...
            if link_mode == "fc": 
                ...
                py5.stroke(180, 100)
                step_val = n_node.get("step_size", 9)
                branch_val = n_node.get("branch_count", 2)
                
                for b in range(branch_val):
                    target_idx = n_node["g_idx"] + b * step_val
                    if target_idx < len(curr_l):
                        py5.line(curr_l[target_idx]["x"], curr_l[target_idx]["y"], n_node["x"], n_node["y"])
...
def draw_nodes():
    ...
    for layer in layers_data:
        for node in layer:
            ...
            if shape == "rect_grid":
                draw_cube(x, y, node["patch_size"], node["num"], c, w, h)
            elif shape == "rect":
                py5.fill(*c)
                py5.stroke(50)
                py5.rect(x, y, w, h) 
                ...
                if heads > 0:
                    ...
                    for h_idx in range(heads):
                        hx = x - w/2 + head_w/2 + h_idx * head_w
                        hy = y - h/2 - head_h/2 
